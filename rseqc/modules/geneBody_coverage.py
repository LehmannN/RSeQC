...
def main():
    ...
    #----- Start modified ----------
    printlog("Get BED files (reference gene model) ...")
    bedfiles = getBamFiles.get_bed_files(options.ref_gene_model)
    gene_percentiles = []
    for bed in bedfiles:
        print bed
        percentiles = genebody_percentile(refbed = bed, mRNA_len_cut = options.min_mRNA_length)
        gene_percentiles.append(percentiles)
    #----- End modified ----------
    ...
    for bamfile in bamfiles:
        ...
        for i in range(len(bedfiles)):
            cvg = genebody_coverage(bamfile, gene_percentiles[i])
            if len(cvg) == 0:
                print >>sys.stderr, "\nCannot get coverage signal from " + basename(bamfile) + ' ! Skip'
                continue
            tmp = valid_name(basename(bamfile).replace('.bam',''))    # scrutinize R identifer
            tmpbed = valid_name(basename(bedfiles[i]).replace('.bed',''))
            if file_container.count(tmp) == 0:
                print >>OUT1, tmp + '.' + tmpbed + '\t' + '\t'.join([str(cvg[k]) for k in sorted(cvg)])
            else:
                print >>OUT1, tmp + '.' + tmpbed + '.' + str(file_container.count(tmp)) + '\t' + '\t'.join([str(cvg[k]) for k in sorted(cvg)])
